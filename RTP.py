...
class RTPClient:
    ...
    def start(self) -> None:
        ...
        self.sout = self.sin  #暂时禁用
        self.sin.bind((self.inIP, self.inPort))
        # self.sin.setblocking(False) 暂时禁用
    ...
    def recv(self) -> tuple[bytes, tuple]:
        while self.NSD:
            try:
                # 设置短暂的超时，让接收操作可以被Ctrl+C中断
                self.sin.settimeout(0.1)
                packet, addr = self.sin.recvfrom(8192)
                return packet, addr # 接收成功
